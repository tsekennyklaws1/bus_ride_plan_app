...
class BusDataManager {
    ...
    func fetchRouteStops(route: String = "" ,direction: String = "", service_type: String = ""){
        if (route != "" && direction != "" && service_type != ""){
            let urlString = "\(busRouteURL)/route-stop/" + (String(route)) + "/" + (String(direction)) + "/" + (String(service_type))
    ...
    {
        if (selectedRoute?.route != nil && selectedRoute?.bound != nil && selectedRoute?.service_type != nil){
            let urlString = "\(busRouteURL)/route-stop/\(String(describing: selectedRoute?.route))/\(selectedRoute?.bound == "O" ? "outbound" : "inbound")/\(String(describing: selectedRoute?.service_type))"
            performRequest(with: urlString, busDataModel_init: BusRouteStopDataModel())
        }
    }
    ...
    func performRequest(with urlString: String, busDataModel_init: BusDataModel!) {
        ...
        var trimmedURL = String(urlString)
        trimmedURL.replace("\"", with: "")
        trimmedURL.replace("(", with: "")
        trimmedURL.replace(")", with: "")
        trimmedURL.replace("Optional", with: "")
        trimmedURL.replace("%22", with: "")
        print("urlString after modified = \(trimmedURL)")
